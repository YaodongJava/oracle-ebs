...
- [实施配置](#implementation) · [工具选型](#2-报表工具选型) · [报表治理](#3-报表治理最小模型) · [关账内控](#4-关账与对账治理) · [架构性能](#6-报表架构开发与性能) · [对账矩阵](#7-关账日历与对账设计) · [审计排错](#8-审计数据保留与隐私) · [验证练习](#10-验证清单) · [页面与关账实操](#13-资深顾问实操报表与关账)
...
报表实施先定义“谁对数字负责、从哪里取数、在什么状态下可以发布”，再选择工具和设计布局。不要将临时 SQL、个人 Excel 或 ECC 索引结果直接定义为法定或月结签核口径；它们可以辅助分析，但必须具备受控的数据源、参数、权限和证据链。

| 顺序 | 配置项 | 预置职责 / 导航（功能名） | 具体配置 | 验收动作 |
| --- | --- | --- | --- | --- |
| 1 | 报表目录与参数契约 | 受控文档库 + 对应产品报表定义 | 为每张报表登记业务所有者、数据源、Ledger/OU 范围、参数、刷新频率、控制总额、保留期和替代报表 | 用一个已关期间填写完整目录记录，并由财务所有者确认口径 |
| 2 | FSG 定义与访问 | `GL Super User > Reports > Define > Row Set / Column Set / Report` | 先维护 Row Set（账户/计算行）和 Column Set（期间、金额类型、计算列），再定义 Report/Content Set；需要时启用 Definition Access Set | 以不同职责运行报表，验证行列、期间偏移、符号、舍入、权限和控制总额 |
| 3 | FSG 运行与发布 | `GL Super User > Reports > Request`；Report Manager 相关功能（如已部署） | 固化 Ledger、币种、期间、Content Set 与输出格式；生产运行应保存 Request ID、参数、输出和发布版本 | 从 GL 余额抽样回算一行 FSG，并确认输出仅发给获授权对象 |
| 4 | BI Publisher 数据与模板 | `XML Publisher Administrator > Home > Data Definitions / Templates`（职责名可能显示为 XML Publisher Administrator） | 创建 Data Definition、数据模板/受控并发程序、Layout Template、Locale、输出格式和数据访问；模板与 SQL/数据模型版本分开编号 | 以同一参数输出 PDF/Excel，核对记录数、控制总额、敏感字段脱敏与模板版本 |
| 5 | Web ADI（如适用） | `Desktop Integration Manager > Define Integrator / Define Layout` | 定义 Integrator、Interface、Layout、LOV、上传权限、必填校验和批次唯一键；明确部分成功行的处理方式 | 上传一份合法和一份包含错误行的工作簿，核对行级错误、重复上传保护和审计批次 |
| 6 | ECC（如已部署） | EBS ECC 管理/产品责任下的 Data Load 并发程序 | 配置职责、数据集、Full/Incremental/Push 加载策略、加载窗口和索引新鲜度提示；ECC 是索引视图，非总账权威余额 | 运行初始加载与增量加载，验证权限过滤、刷新时间和与来源交易的抽样一致性 |
| 7 | 关账任务与签核 | 关账日历/任务台账；`View > Requests` 和模块关期功能 | 将前置请求、对账、关期、报表、签核分为独立任务，定义负责人、截止时间、依赖、例外升级和证据路径 | 模拟一项失败的前置请求，确认后续关期/签核不能被误标为完成 |
| 8 | 权限与审计 | `System Administrator > Security > Responsibility > Define`；`Profile > System` | 最小权限授予报表职责、Definition Access Set、Ledger/OU 数据访问和输出目录访问；定期复核职责与敏感报表订阅 | 对“制表人、复核人、审批人”三类用户执行访问测试并保存截图/日志 |

### 发布门禁

报表发布需同时满足：来源期间/状态正确、参数完整、控制总额通过、版本已批准、权限已验证、输出已归档。报表格式变更、SQL/数据模板变更、FSG 行列变更及安全范围变更均要重新执行该门禁；仅修改“显示格式”也可能改变舍入、分页或敏感信息暴露。
